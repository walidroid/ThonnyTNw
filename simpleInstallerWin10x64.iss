...
; 2. Installation groupée (Thonny + PyInstaller + Dépendances)
; L'utilisation de -r requirements.txt permet d'installer PyInstaller et ses dépendances (altgraph, etc.) en une fois.
...
Filename: "cmd.exe"; Parameters: "/q /c mode 80,5 && title Installation des outils spécifiques ... && {tmp}\RefreshEnv.cmd && py.exe -m pip install thonny-tunisiaschools thonny-autosave --upgrade --no-index --prefer-binary --find-links {tmp}\deps >> {tmp}\innosetup.log"; StatusMsg: "Configuration finale des plugins..."; Components: "editors"
